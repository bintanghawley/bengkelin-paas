<div id="booking-modal-{{ $booking->id }}" class="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] hidden backdrop-blur-sm p-4 overflow-y-auto" onclick="toggleBookingModal({{ $booking->id }}, false)">
    <div class="bg-zinc-900 border border-zinc-800 w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl relative my-8" onclick="event.stopPropagation()">
        <!-- Close Button -->
        <button type="button" onclick="toggleBookingModal({{ $booking->id }}, false)" class="absolute top-6 right-6 text-zinc-400 hover:text-white transition">
        ...
        <!-- Modal Header -->
        <div class="p-6 border-b border-zinc-800/80 flex items-center justify-between pr-16">
            <div>
                <h3 class="text-xl font-bengkel uppercase tracking-widest text-white">Detail Booking #{{ $booking->id }}</h3>
                <p class="text-zinc-500 text-xs mt-1 uppercase tracking-widest">Sidoarjo High Performance Garage</p>
            </div>
            @php
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(100vh-12rem)]">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                <!-- Left: Info Utama -->
                <div class="lg:col-span-1 bg-zinc-950/40 border border-zinc-800/50 rounded-2xl p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold text-white uppercase mt-1 text-left">{{ $booking->user->name ?? 'Guest' }}</p>
                        <p class="text-[10px] text-zinc-500 font-mono text-left">{{ $booking->user->nomor_telepon ?? '-' }}</p>
                    </div>

                    <div>
                        <p class="text-[10px] text-zinc-400 uppercase font-bold tracking-widest">Layanan</p>
                        <p class="text-sm font-bold text-white uppercase mt-1 text-left">{{ $booking->service->nama }}</p>
                        <p class="text-xs text-zinc-500 text-left">Estimasi: {{ $booking->service->estimasi_waktu }}</p>
                    </div>

                    <div>
                        <p class="text-[10px] text-zinc-400 uppercase font-bold tracking-widest">Jadwal</p>
                        <p class="text-sm font-semibold text-white mt-1 text-left">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold text-white uppercase mt-1 text-left">{{ $booking->nama_kendaraan }}</p>
                        <p class="text-xs text-zinc-500 font-mono text-left">{{ $booking->plat_nomor }}</p>
                    </div>

                    @if($booking->mechanic)
                    <div>
                        <p class="text-[10px] text-zinc-400 uppercase font-bold tracking-widest">Mekanik Bertugas</p>
                        <p class="text-sm font-bold text-white uppercase mt-1 text-left">{{ $booking->mechanic->name }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    <!-- Keluhan -->
                    <div class="bg-zinc-950/40 border border-zinc-800/50 rounded-2xl p-6">
                        ...
                        <p class="text-zinc-300 text-sm leading-relaxed whitespace-pre-line bg-zinc-950 p-4 rounded-xl border border-zinc-800 normal-case text-left">
                            {{ $booking->keluhan ?: 'Tidak ada keluhan tertulis.' }}
                        </p>
                    </div>

                    <!-- Aksi Mekanik berdasarkan status -->
                    <div class="bg-zinc-950/40 border border-zinc-800/50 rounded-2xl p-6 space-y-4">
                        ...
                        @if($booking->status === 'pending')
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch pt-2">
                                {{-- Terima Card --}}
                                <div class="bg-zinc-950/60 p-6 rounded-2xl border border-zinc-800/80 flex flex-col justify-between">
                        ...
                        @elseif($booking->status === 'diterima' && $booking->mechanic_id === auth()->id())
                            {{-- Start work --}}
                            <p class="text-blue-400 text-xs normal-case text-left mb-2">Booking sudah diterima. Klik tombol di bawah untuk mulai pengerjaan servis.</p>
                        ...
                        @elseif($booking->status === 'diproses' && $booking->mechanic_id === auth()->id())
                            {{-- Complete work --}}
                            <p class="text-yellow-400 text-xs normal-case text-left mb-2">Sedang dikerjakan. Isi catatan pekerjaan dan klik selesai bila sudah rampung.</p>
                            <form action="{{ route('mekanik.bookings.update', $booking->id) }}" method="POST" class="space-y-4">
                                @csrf @method('PUT')
                                ...
                                <div class="space-y-2 text-left">
                                    ...
                                    <textarea name="catatan_mekanik" required rows="5"
                                        ...
                                        class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-sm text-white focus:border-emerald-600 outline-none transition normal-case text-left">{{ $booking->catatan_mekanik }}</textarea>
                                </div>
                                <button type="submit"
                        ...
                        @elseif($booking->status === 'selesai')
                            <div class="bg-emerald-900/20 border border-emerald-800 rounded-xl p-5 text-left">
                        ...
                        @elseif($booking->status === 'ditolak')
                            <div class="bg-red-900/20 border border-red-800 rounded-xl p-5 text-left">
                        ...
                        @else
                            <p class="text-zinc-500 text-xs normal-case italic text-left">Tidak ada aksi yang tersedia untuk status ini.</p>
                        @endif
                    </div>
                </div>
            </div>
